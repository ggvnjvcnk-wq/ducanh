...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $insert_success = $DVR->update("news", [
        'tieude'       => $tieude,
        'link'         => xoadau($tieude),
        'home' => $home,
        'image_news'   => $image,
    ...
    if ($insert_success) {
        ...
        $DVR->insert("logs", [
            ...
            'create_date'   => gettime(),
            'action'        => 'Cập nhật bài viết thành công (#' . $tieude . ')'
        ]);

        // Send Telegram notification
        sendTele(templateTele($getUser['username'] . " Cập nhật bài viết thành công (#" . $tieude . ")"));

        die(json_encode(['status' => 'success', 'msg' => 'Cập nhật bài viết thành công']));
    } else {
        die(json_encode(['status' => 'error', 'msg' => 'Không Cập nhật được bài viết']));
    }
