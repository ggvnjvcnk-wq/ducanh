...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
   ...
   if (check_xss($_POST['title']) || check_xss($_POST['image']) || check_xss($_POST['money']) || check_xss($_POST['sdt']) || check_xss($_POST['email']) || check_xss($_POST['telegram']) || check_xss($_POST['linkfb']) || check_xss($_POST['idfb']) || check_xss($_POST['gioi_thieu'])) {
        die(json_encode(['status' => 'error', 'msg' => 'DichVuRight Xin Chào !!!']));
    }
    ...
    if ($insert_success) {
        ...
        $DVR->insert("logs", [
            ...
            'create_date'   => gettime(),
            'action'        => 'Cập Nhật GDV thành công (#' . $title . ')'
        ]);

        // Send Telegram notification
        sendTele(templateTele($getUser['username'] . " Cập Nhật GDV thành công (#" . $title . ")"));

        die(json_encode(['status' => 'success', 'msg' => 'Cập Nhật thành công']));
    } else {
        die(json_encode(['status' => 'error', 'msg' => 'Lỗi Không Cập Nhật Được GDV']));
    }
