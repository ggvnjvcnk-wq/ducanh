...
if (isset($_POST['id'])) {
    ...
    $row = $DVR->get_row("SELECT * FROM `doitac` WHERE `id` = '$id' ");
    if (!$row) {
        $data = json_encode([
            'status'    => 'error',
            'msg'       => 'Đối Tác không tồn tại trong hệ thống'
        ]);
        die($data);
    }
    $isRemove = $DVR->remove("doitac", " `id` = '$id' ");
    if ($isRemove) {
        $DVR->insert("logs", [
            'user_id'       => $getUser['id'],
            'ip'            => myip(),
            ...
            'create_date'    => gettime(),
            'action'        => 'Xóa Đối Tác ('.$row['id'].') '
         ]);
        $data = json_encode([
            'status'    => 'success',
            'msg'       => 'Xóa Đối Tác thành công'
        ]);
        die($data);
    }
} else {
